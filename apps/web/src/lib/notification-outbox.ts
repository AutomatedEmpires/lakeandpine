...
export async function retryOutboxNotification(
    ...
    const rows = await tx<ClaimedNotification[]>`
      with claimed as (
        ...
        where o.id = ${outboxId}
          ...
          and (
            o.status in ('pending', 'retry', 'failed')
      ...
      select o.id, o.locked_at::text as claim_locked_at,
        o.booking_id, o.service_case_id, o.notification_type,
  ...
  await withStaffActor(customerId, async (tx) => {
    const rows = await tx<{ id: string }[]>`
      update notification_outbox set status = ${status}, attempt_count = attempt_count + 1,
      ...
      where id = ${item.id} and status = 'processing'
        and locked_at = ${item.claim_locked_at}::timestamptz
      returning id`;
    if (!rows[0]) throw new Error("Notification retry claim is no longer active");
  });
  return outcome;
}
