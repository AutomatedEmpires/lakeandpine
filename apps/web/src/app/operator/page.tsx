            ...
            <div className="operator-workbench">
              ...
              <div className="operator-detail-stack">
                ...
                <section className="card operator-panel">
                  ...
                  <div className="internal-note-list">{notes.map((note) => <article key={note.id}><div><strong>{note.author_label}</strong><span>{new Date(note.created_at).toLocaleString("en-US", { timeZone: selected.territory_timezone, month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}</span></div><p>{note.body}</p></article>)}{notes.length === 0 && <p className="copy">No internal notes yet.</p>}</div>
                </section>
